...
router.post("/create", isTeacher, async (req, res) => {
  try {
    ...
    const result = await client.query(
      "SELECT * FROM months WHERE id = $1 AND teacher_id = $2",
      [m_id, teacher_id]
    );
    ...
    if (!flag) {
      return res
        .status(403)
        .json({ msg: "You must send a valid month associated with you." });
    }
    ...
    for (let i = 0; i < number; i++) {
      ...
      await client.query(
        "INSERT INTO teachercode (code, m_id, teacher_id) VALUES ($1, $2, $3)",
        [code, m_id, teacher_id]
      );
      codes.push(code);
    }
    ...
    res.json({ name: result.rows[0].description, codes });
  } catch (error) {
    console.error(error);
    ...
    res
      .status(500)
      .json({ msg: "An error occurred while creating the codes." });
  }
});
